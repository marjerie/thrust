...
struct vm_copy_allocator_t {};

// template<typename T, typename Alloc>
// class vm_storage_base
// {
//   private:
...
// template<typename T>
// class vm_storage_base<T, thrust::virtual_allocator<T>>
// {
//   private:
...
template<typename T, typename Alloc>
  class vm_storage //: public vm_storage_base<T, Alloc>
{
  private:
    typedef thrust::detail::allocator_traits<Alloc> alloc_traits;
  ...
  public:
    ...
    __host__ __device__
    explicit vm_storage(const allocator_type &alloc = allocator_type());

    __thrust_exec_check_disable__
    __host__ __device__
    explicit vm_storage(size_type n, const allocator_type &alloc = allocator_type());

    __thrust_exec_check_disable__
    __host__ __device__
    explicit vm_storage(vm_copy_allocator_t, const vm_storage &other);

    __thrust_exec_check_disable__
    __host__ __device__
    explicit vm_storage(vm_copy_allocator_t, const vm_storage &other, size_type n);
    ...
    __host__ __device__
    void deallocate_on_allocator_mismatch(const vm_storage &other);

    __host__ __device__
    void destroy_on_allocator_mismatch(const vm_storage &other,
    ...
    __host__ __device__
    bool is_allocator_not_equal(const vm_storage &other) const;

    __host__ __device__
    void propagate_allocator(const vm_storage &other);

#if THRUST_CPP_DIALECT >= 2011
    __host__ __device__
    void propagate_allocator(vm_storage &other);

    // allow move assignment for a sane implementation of allocator propagation
    // on move assignment
    __host__ __device__
    vm_storage &operator=(vm_storage &&other);
#endif
  ...
  private:
    ...
    // disallow assignment
    vm_storage &operator=(const vm_storage &x);
    ...
    __host__ __device__
    void deallocate_on_allocator_mismatch_dispatch(true_type, const vm_storage &other);

    __host__ __device__
    void deallocate_on_allocator_mismatch_dispatch(false_type, const vm_storage &other);

    __host__ __device__
    void destroy_on_allocator_mismatch_dispatch(true_type, const vm_storage &other,
        iterator first, iterator last);

    __host__ __device__
    void destroy_on_allocator_mismatch_dispatch(false_type, const vm_storage &other,
        iterator first, iterator last);

    __host__ __device__
    void propagate_allocator_dispatch(true_type, const vm_storage &other);

    __host__ __device__
    void propagate_allocator_dispatch(false_type, const vm_storage &other);

#if THRUST_CPP_DIALECT >= 2011
    __host__ __device__
    void propagate_allocator_dispatch(true_type, vm_storage &other);

    __host__ __device__
    void propagate_allocator_dispatch(false_type, vm_storage &other);
#endif
}; // end vm_storage
...
#include <thrust/detail/vm_storage.inl>
